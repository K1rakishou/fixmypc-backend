...
class ImageController {
    ...
    @RequestMapping(path = arrayOf("${Constant.Paths.IMAGE_CONTROLLER_PATH}/{user_id}/{size}/{image_name:.+}"),
            method = arrayOf(RequestMethod.GET))
    fun serveImage(@PathVariable("image_name") imageName: String,
                   @PathVariable("size") size: String,
                   @PathVariable("user_id") userId: Long): Single<ResponseEntity<Resource>> {

        return imageService.serveImage(userId, imageName, size)
                .map { result ->
                    when (result) {
